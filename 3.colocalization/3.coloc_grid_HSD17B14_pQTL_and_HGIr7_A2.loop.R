...
setwd("/scratch/richards/satoshi.yoshiji/09.proMR/24.colocalization_grid/1.HSD17B14_HGIr7_A2/27combinations/")
...
trait1_gwas <- vroom('/scratch/richards/public/decode_proteomics_2021/13972_4_HSD17B14_DHB14.txt.gz')
trait1_sel <- trait1_gwas %>% filter(Chrom == 'chr19') %>% filter(Pos >= (48815123 - 500000)) %>% filter(Pos <= (48815123 + 500000))
...
trait2_gwas <- vroom('/scratch/richards/satoshi.yoshiji/09.proMR/disease_GWAS/COVID/HGI_r7/COVID19_HGI_A2_ALL_eur_leave23andme_20220403.tsv')
trait2_sel <- trait2_gwas %>% filter(`#CHR` == '19') %>% filter(POS >= (48815123 - 500000)) %>% filter(POS <= (48815123 + 500000))
...
trait1_common <- trait1_nodup %>% filter(id %in% common_id) %>% arrange(position)
trait2_common <- trait2_nodup %>% filter(id %in% common_id) %>% arrange(position)

#trait1_common$chr <- 4
write.table(trait1_common , file = 'trait1_1Mb_around_rs35299026.tsv', row.names = F, sep = '\t', quote = F)

#trait2_common$chr <- 4
write.table(trait2_common , file = 'trait2_1Mb_around_rs35299026.tsv', row.names = F, sep = '\t', quote = F)
...
trait2_list$type <- 'cc'
trait2_list$N <- 13769+1072442
trait2_list$cc <- 13769/(13769+1072442)
str(trait2_list)
...
coloc_res <- coloc.abf(
  ...
  p1 = p1, p2 = p2, p12 = p12,
  #p1 = 1e-04, p2 = 1e-04, p12 = 1e-05 # option default
) 
